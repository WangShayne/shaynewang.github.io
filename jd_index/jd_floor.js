
function getTop(elem){
	//声明变量sum.初始为elem的offsetParent
	var sum=elem.offsetTop
	//反复判断elem的offsetParent不等于null
	while(elem.offsetParent){
		//将elem.offsetParent的offsetTop累加到sum上
		sum+=elem.offsetParent.offsetTop
		//将elem换成elem的offsetParent
		elem=elem.offsetParent
		}	//遍历结束
		return sum//返回sum
}
window.addEventListener("load",function(){floor.init()});
var floor={
	UPLEVEL:0,//亮灯范围的上限 距文档显示区的距离
	DOWNLEVEL:0,//亮灯范围的下线, 距文档显示区的距离

	distance:0,//用来保存本次移动的总距离 
	DURATION:0,//保存本次移动的总时间
	STEPS:100,//保存本次移动的总步数
	moved:0,//保存本次已经移动的步数,控制移动结束
	step:0,//保存每步移动的步长
	INTERVAL:0,//保存每步移动的时间间隔
	timer:null,//本次移动的序号

	init:function(){
		//获得id为f1的div,计算后的样式的height,转为浮点数保存在变量fHeight中
		var fHeight=parseFloat(getComputedStyle($("#f1")).height);
		//计算UPLEVEL:(文档显示区的高-fHeight)/2
		this.UPLEVEL=(window.innerHeight-fHeight)/2;
		//计算DOWNLEVEL:UPLEVEL+fHeight
		this.DOWNLEVEL=this.UPLEVEL+fHeight;
		//为window绑定onscroll事件,提前绑定this
		window.addEventListener("scroll",this.checkLight.bind(this));
		//为id为elevator的div下的ul,绑定鼠标进入事件
		$("#elevator>ul").addEventListener("mouseover",this.showEtitle);
		//为id为elevator的div下的ul,绑定鼠标移除事件
		$("#elevator>ul").addEventListener("mouseout",this.hideEtitle)
		//计算INTERVAL为DURATION/STEPS
		this.INTERVAL=this.DURATION/this.STEPS;
		//为id为elevator的div下的ul,绑定单击事件为move,提前绑定this
		$("#elevator>ul").addEventListener("click",this.move.bind(this))
	},
	move:function(e){
		//如果目标元素是a
		if(e.target.nodeName=="A"&&e.target.className=="etitle"){
			//如果timer不是null,就停止动画//timer=null moved归零
			if(this.timer!=null){clearTimeout(this.timer),this.timer=null,this.moved=0};
			//获得当前网页滚动的scrollTop
			var scrollTop=document.body.scrollTop;
			//获得目标元素父元素下的第一个a元素的内容,转为整数,保存在变量i中
			var i=parseInt(e.target.parentNode.firstElementChild.innerHTML);
			//查找fi下的header下的span,保存在span中
			var span=$("#f"+i+">header>span");
			//获得span距离页面顶部的距离getTop,保存在totalTop中
			var totalTop=getTop(span);
			//计算distance为:totalTop-UPLEVEL-scrollTop
			this.distance=totalTop-this.UPLEVEL-scrollTop;
			//计算step为:distance/STEPS
			this.step=this.distance/this.STEPS
			//启动一次性定时器,传入moveStep作为任务函数,提前绑定this,时间间隔设置为INTERVAL,将序号保存在timer中
			this.timer=setTimeout(this.moveStep.bind(this),this.INTERVAL);
		}	
	},
	moveStep:function(){//移动一步
		//让当前窗口滚动一个step
		scrollBy(0,this.step);
		//将moved+1;
		this.moved++;
		//如果moved<STEPS
		if(this.moved<this.STEPS){
			///启动一次性定时器,传入moveStep作为任务函数,提前绑定this,时间间隔设置为INTERVAL,将序号保存在timer
			this.timer=setTimeout(this.moveStep.bind(this),this.INTERVAL);
		}
		//否则
		else{
			//停止定时器,timer置为null,moved归0
			clearTimeout(this.timer);
			this.timer=null;
			this.moved=0;
		}
	},
	showEtitle:function(e){//显示当前li下的etitle的a
		//先获得目标元素target
		var target=e.target;
		//如果target是a的话
		if(target.nodeName=="A"){
			//将target变为target的父节点(li)
			target=target.parentNode;
		}
			//如果target是li的话
		if(target.nodeName=="LI"){
			//找到target下的第一个a,将其隐藏
			target.firstElementChild.style.display="none";
			//在target下找第二个a,将其显示
			target.firstElementChild.nextElementSibling.style.display="block";
		}
	},
	hideEtitle:function(e){//隐藏当前li下的etitle的a
		//先获得目标元素target
		var target=e.target;
		//如果target是a的话
		if(target.nodeName=="A"){
			//将target变为target的父节点(li)
			target=target.parentNode;
		}
			//如果target是li的话
		if(target.nodeName=="LI"){
			//获得target下第一个a元素的内容,转为整数,保存在变量i中
			var i=parseInt(target.$("a:first-child").innerHTML);
			 //id为f+i下的header的span
			var span=$("#f"+i+">header>span");
				//如果span的class不是hover 
				if(span.className!="hover"){
					//找到target下的第一个a,将其显示
					target.$("a:first-child").style.display="block";
					//在target下找第二个a,将其隐藏
					target.$("a:first-child+a").style.display="none";
				}
		}
	},
	checkLight:function(){//检查每个楼层中span的量/灭的状态
		//获得当前页面的滚动距离,保存在变量scrollTop中
		var scrollTop=document.body.scrollTop;
		//获得所有class为floor下的header下的直接子节点span,将查找到的结果保存在变量span中
		var span=$(".floor>header>span");
		//遍历span中每个span,
		for(var i=0;i<span.length;i++){
			//用getTop方法获得当前span距离页面顶部的总距离,保存在变量totalTop中
			var totalTop=getTop(span[i]);
			//用totalTop-scrollTop,将结果保存在变量innerTop中
			var innerTop=totalTop-scrollTop;
			//查找id为elevator下的ul下的所有li中下标为i的li,保存在变量li中
			var li=$("#elevator>ul>li")[i];
			//在li下找第一个a,保存在变量a1中,
			a1=li.$("a:first-child")
			//在li下找第二个a,保存在变量a2中,
			a2=li.$("a:first-child+a")
			//如果innerTop>=UPLEVEL,且innerTop<=DOWNLEVEL
			if(innerTop>=this.UPLEVEL&&innerTop<=this.DOWNLEVEL){
				span[i].className="hover";//将当前span的class设置为hover
				//设置a1隐藏,a2显示
				a1.style.display="none";
				a2.style.display="block";
			}
			else{//否则
				//清除当前span 的calss
				span[i].className="";
				//设置a1显示,a2隐藏
				a1.style.display="block";
				a2.style.display="none";
			}
		}
		//查找class为floor下的header下的class为hover的span,保存在变量lightSpan中
		var lightSpan=$(".floor>header>span.hover");
		//设置id为elevator的div的display,如果lightSpan不是null,就设置为block,否则设置为none
		$("#elevator").style.display=lightSpan!=null?"block":"none"
	},

}